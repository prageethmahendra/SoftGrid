...
class SimAuto:
	...
	def SolvePowerFlow(self, Method=POWER_FLOW_METHOD_RECTNEWT):
	...
	def TSSetStorageOption(self, ObjectType, FieldList=[], IsEnable=True):
		...
		if FieldList == []:
			Script = "TSResultStorageSetAll(%s, %s);"% \
				(ObjectType, STATUS_YES if IsEnable else STATUS_NO)
			...
			for data in Data:
				for i in range(FieldNum):
					data[-(1+i)] = STATUS_YES if IsEnable else STATUS_NO
	...
	def TSRunAndPause(self, CtgName, StopTime, IsReset=False):
		assert isinstance(CtgName, str)
		assert isinstance(StopTime, str)
		...
		ResetStartTime = STATUS_YES if IsReset else STATUS_NO
