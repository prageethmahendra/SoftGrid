...
import it.adsc.smartpower.substation.monitoring.ui.config.ConfigPanel;
import it.adsc.smartpower.substation.monitoring.ui.alerts.MainAlert;
import it.adsc.smartpower.substation.monitoring.ui.message.MessageUIHandler;
...
//          logTextArea.updateUI();
    ...
    private ChartPanel getChartPanel() {
        final JFreeChart chart = ChartFactory.createXYLineChart(
                "SmartPower Monitor",   // chart title
        ...
        pythonThread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    File batFile = new File(ConfigUtil.PYTHON_START_BAT_FILE_PATH);
                    FileWriter fw = null;
                    try {
                        if (!batFile.exists()) {
                    ...
                    startPythonThread(new File(ConfigUtil.PYTHON_START_BAT_FILE_PATH).getAbsolutePath());
                } catch (Exception e1) {
                    e1.printStackTrace();
                }
            }
...
//      destroyProcess(pythonProcess);
        File file = new File("..\\SmartPower\\casefiles\\state.file");
        file.delete();
        try {
            Thread.sleep(500);
